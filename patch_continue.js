...
// The regex matches the start of the MY LIBRARY section
const libraryStartRegex = /\{\/\* ── MY LIBRARY ──────────────────────────────────────────────────────── \*\/\}\n\s+<\!isReading && \!loading && library\.length > 0 && \(\n\s+<div className="max-w-3xl mx-auto mt-8 mb-16 space-y-4 animate-fade-in">/;
      ...
      {!isReading && !loading && library.length > 0 && (
        <div className="max-w-3xl mx-auto mt-8 mb-16 space-y-8 animate-fade-in">
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-700 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden group cursor-pointer hover:scale-[1.01] transition-transform" onClick={() => loadFromLibrary(library[0].name)}>
            <div className="absolute top-0 right-0 -mt-8 -mr-8 w-48 h-48 bg-white opacity-5 rounded-full blur-3xl group-hover:opacity-10 transition-opacity" />
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-2 text-slate-800 dark:text-slate-200">
              ...
              <span className="text-xs font-semibold text-slate-500 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">
                {library.length} {library.length === 1 ? "Book" : "Books"} Stored (Up to 5GB+)
              </span>
            </div>
...
const replacementRegex = /\{\/\* ── MY LIBRARY ──────────────────────────────────────────────────────── \*\/\}\n\s+<\!isReading && \!loading && library\.length > 0 && \([\s\S]*?<\/\div>\n\s+\)\}/;

code = code.replace(replacementRegex, newLibraryUI);
